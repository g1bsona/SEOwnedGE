...
void CMovementSimulation::SetupMoveData(C_TFPlayer* pPlayer, CMoveData* pMoveData)
{
	...
	else if (CFG::Aimbot_Projectile_Aim_Prediction_Method == 2) // Clamp and Normalize method
	{
		...
		// TF_COND_SLOWED doesn't exist, using a different approach for slow effects
		// Instead of using (1 << TF_COND_SPEED_BOOST + 1) which causes overflow
		// Look for other conditions that indicate slowdown
		if (pPlayer->InCond(TF_COND_STUNNED) || pPlayer->m_flStunLerpTarget() > 0.0f || pPlayer->m_bStunNeedsFadeOut())
		{
